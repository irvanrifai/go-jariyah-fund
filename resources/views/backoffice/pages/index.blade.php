...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <table id="daftar-halaman-table" class="table table-bordered table-striped table-hover">
                        <thead>
                        <tr>
                            <th>Judul</th>
                            <th>Status</th>
                        ...
                        @foreach($list as $li)
                            <tr id="row-num-{{ $li['id'] }}">
                                <td>
                                    <a href="/backoffice/pages/{{ $li['id'] }}/edit">{{ $li['title'] }}</a><br>
                                </td>
                                <td>{{ $li['status'] }}</td>
                                <td>
                                    <a target="_blank" class="btn btn-link btn-sm text-info"
                                       href="/p/{{ $li['slug'] }}/?preview=1"><i class="fas fa-eye"></i>&nbsp;
                                        Lihat</a> &nbsp;
                                    <a class="btn btn-link btn-sm text-primary"
                                       href="/backoffice/pages/{{ $li['id'] }}/edit"><i class="fas fa-pen-fancy"></i>&nbsp;
                                        Edit</a> &nbsp;
...
@section('adminlte_js')
    ...
    <script>
        $(document).ready(() => {
            $("#daftar-halaman-table").DataTable({
                order: [0, 'asc'],
                responsive: true
            });
        ...
        function formHapus(id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: '{{route('backoffice.pages.destroy','')}}/' + id,
                        type: 'POST',
                        data: {
                            '_method': 'DELETE',
                            '_token': '{{ csrf_token() }}',
                        ...
                        success: function () {
                            $("#daftar-halaman-table").DataTable().row('#row-num-'+id).remove().draw();
                            Swal.fire({
                                icon: 'success',
