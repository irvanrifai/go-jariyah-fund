                ...
                <div class="card-body">
                    <table id="slider-table" class="table table-bordered table-striped table-hover">
                        ...
                        @foreach($slider as $li)
                            <tr id="row-num-{{ $li['id'] }}">
                                <td>
                                    <a href="/backoffice/slider/{{ $li['id'] }}">{{ $li['title'] }}</a><br>
                                </td>
                                ...
                                <td>
                                    <a class="btn btn-link btn-sm text-info"
                                       href="/backoffice/slider/{{ $li['id'] }}"><i class="fas fa-eye"></i>&nbsp;
                                        </a> &nbsp;
                                    <a class="btn btn-link btn-sm text-primary"
                                       href="/backoffice/slider/{{ $li['id'] }}/edit"><i class="fas fa-pen-fancy"></i>&nbsp;
                                        </a> &nbsp;
    ...
    <script>
        ...
        function formHapus(id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: '{{route('backoffice.slider.destroy','')}}/' + id,
                        type: 'POST',
                        data: {
                            '_method': 'DELETE',
                            '_token': '{{ csrf_token() }}',
                        ...
                        success: function () {
                            $("#slider-table").DataTable().row('#row-num-'+id).remove().draw();
                            Swal.fire({
                                icon: 'success',
