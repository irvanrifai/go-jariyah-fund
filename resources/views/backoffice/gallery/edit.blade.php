...
@section('title', 'Sunting Gambar')

@section('content_header_title','Sunting Gambar')
@section('content_header_prev_link','/backoffice/gallery')
@section('content_header_prev_text','Semua Gambar')

@section('content')
    <form class="card" action="{{ route('backoffice.gallery.update', $gallery->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('patch')

        <div class="card-header d-flex align-items-center">
            <h3 class="card-title">Gallery: Sunting Gambar</h3>

            <div class="card-tools ml-auto mr-0">
                <a href="{{ route('backoffice.gallery.index') }}" class="btn btn-secondary btn-sm" data-toggle="tooltip" title="Kembali ke Gallery">
        ...
        <div class="card-body">
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" id="input-title" placeholder="Judul" value="{{ $gallery->title }}" required>
                    @error('title')
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <textarea class="form-control" name="desc" placeholder="Deskripsi" rows="4">{{ $gallery->desc ?? '-' }}</textarea>
                </div>
            </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-4">
                    <div class="input-group">
                        ...
                        <input id="thumbnail" type="text" name="image" class="form-control @error('image') is-invalid @enderror" value="{{$gallery->image}}" readonly>
                    </div>
                    @error('image')
                    <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                ...
                <span class="input-group-btn">
                    ...
                    <input type="checkbox" name="is_show" data-plugin="switchery" data-color="#1bb99a" {{ $gallery->is_show ? 'checked' : ''}} >
                </span>
