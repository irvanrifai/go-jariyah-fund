...
@section('content_header_title','Sunting Artikel')
@section('content_header_prev_link','/backoffice/posts')
@section('content_header_prev_text','Semua Artikel')

@section('plugins.Summernote', true)

@section('content')
    <form class="main-form" action="{{ route('backoffice.posts.update',$post->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="form-group">
                            <input name="title" placeholder="Judul Artikel Anda" type="text" class="form-control"
                                   id="titleArticle"
                                   aria-describedby="helperTitle" value="{{$post->title}}"><br>
                            <div class="input-group input-group-sm mb-3">
                                <div class="input-group-prepend">
                                    <span class="input-group-text"
                                          id="slug-url-desc">{{ env('APP_URL') }}/artikel/</span>
                                </div>
                                <input name="slug" type="text" class="form-control" id="slugURL"
                                       aria-describedby="slug-url-desc"
                                       placeholder="URL artikel akan digenerate judul artikel." value="{{$post->slug}}"
                                       readonly>
                            </div>
                        </div>
                        <div class="form-group">
                            <textarea id="masterContent" name="main_content">{!! $post->content !!}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-3">
                <div class="form-group">
                    ...
                    <select class="form-control select2" id="mainStatus" name="status">
                        <option value="public" {{ $post->status==="public" ? 'selected' : '' }}>Publik</option>
                        <option value="private" {{ $post->status==="private" ? 'selected' : '' }}>Pribadi</option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="mainCategory">Kategori</label>
                    <?php $list = \App\Http\Controllers\PostsController::get_categories() ?>
                    <select class="form-control select2" id="mainCategory" name="category">
                        @foreach($list as $li)
                            {{ $get_cat = \App\Http\Controllers\PostsController::get_post_category($post->id,'id')  }}
                            <option
                                value="{{ $li['id'] }}" {{ $get_cat===$li['id'] ? 'selected' : '' }}>{{ $li['title'] }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <div class="input-group">
                       <span class="input-group-btn">
                        ...
                        <input id="thumbnail" class="form-control" type="text" value="{{$post->featured_image}}"
                ...
                <div class="form-group">
                    ...
                    <div class="collapse" id="deleteItem">
                        <button data-id="{{$post->id}}" type="button" class="btn btn-danger btn-sm btn-block"
    ...
    <form id="delete-form" action="{{route('backoffice.posts.destroy',$post->id)}}" method="POST"
          style="display: none;">
        @method('DELETE')
