...
@section('body')
    <div class="content">
        <div class="pl-2 pt-2">
            <h4 class="p-2">Data Pengajuan Pinjam <span class="text-muted">(Nazhir Section)</span></h4>
        </div>
    ...
    <script>
        const table = $('#myTable2').DataTable({
            ...
            responsive: true,
            ajax: {
                url: "{{ route('admin.approval-nazhir.data-approve') }}",
                type: "GET"
            },
            columns: [{
    ...
    <script>
        $(document).ready(function(){
            $('#nazhir_id').select2({
                theme: 'bootstrap4',
                placeholder: "--Pilih Nazhir--",
                allowClear: true,
                ajax: {
                    url: "{{ route('admin.approval-nazhir.select2-nazhir') }}",
                    delay: 100,
    ...
    <script>
        $(document).on('click', '.detail-ajuan', function(e) {
            ...
            var id = $(this).data('id');
            // console.log(id);
            $('#add-edit-approval-modal').modal('show');
            $.ajax({
                type: "get",
                url: "{{ URL::to('admin/approval-nazhir/detail-request-pinjam-anggota') }}" + "/" + id,
                success: function(response) {
                    // console.log(response.data.data_pinjam);
                    // console.log(response.data.data_pinjam_approval_nazhir);
                    if (response.status != 200) {
                        ...
                        $('#nominal_request').val('Rp. ' + response.data.data_pinjam.nominal_request
                        ...
                        $('#date_request').val(response.data.data_pinjam.created_at.toLocaleString(
                            'id-ID'));
                        $('#cicilan_perbulan').val('Rp. ' + response.data.data_pinjam.cicilan_perbulan
                            .toLocaleString('id-ID'));
                        $('#cicilan_pokok').val('Rp. ' + response.data.data_pinjam.cicilan_pokok
                            .toLocaleString('id-ID'));
                        $('#cicilan_modharabah').val('Rp. ' + response.data.data_pinjam
                            .cicilan_modharabah.toLocaleString('id-ID'));
                        $('#total_mudharabah').val('Rp. ' + response.data.data_pinjam.total_mudharabah
                            .toLocaleString('id-ID'));

                        $('#nominal_accepted').val('Rp. ' + response.data.data_pinjam.nominal_accepted
                            .toLocaleString('id-ID'));
                        ...
                        $('#nazhir_id').empty().trigger('change');
                        $('#status_next').val('');
                        $('#note').val('');
                        ...
                        var contentTable = '';
                        $.each(response.data.data_pinjam_approval_nazhir, function (i, item) {
    ...
    <div class="modal fade" id="add-edit-approval-modal" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                        ...
                        <form action="javascript:void(0)" method="POST" enctype="multipart/form-data" name="form-add-edit-approval" id="form-add-edit-approval">
                        @csrf
                        ...
                        <div class="row">
                            <div class="col-lg-10">
                                <h5 class="text-bold">Perijinan Anda <span
                                        class="text-muted text-small">(Nazhir)</span> :</h5>
                            </div>
                        </div>
                        ...
                        <div class="row" id="fill-table-status">
                            <div class="col-lg-4">
                                <div class="form-group">
                                    <label for="nazhir_id" class="col-form-label">Nama Nazhir</label>
                                    <select name="nazhir_id" id="nazhir_id" class="form-control" required>
                                        <option value="" selected disabled>Pilih</option>
                                    </select>
                                    <div id="nazhir-validation" class="text-danger"></div>
                                </div>
                            </div>
                            <div class="col-lg-3">
                                <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-lg-12" id="table-data-approval">
                                    <div class="pl-2 pt-2">
                                        <h6>Data Approval<span class="text-muted"> By Nazhir</span></h6>
                                    </div>
                                    <div class="card demo-icons" style="margin-top:20px;">
                                        <div class="card-body">
                ...
                <div class="modal-footer">
                    ...
                    {{-- <button type="button" class="btn btn-warning edit-approval">Test modal edit</button> --}}
        ...
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" id="submit-btn" class="btn btn-primary submit">Save</button>
                </div>
                </form>
            </div>
        </div>
...
<script>
    $('#submit-btn').click(function(e) {
        ...
        $.ajax({
            ...
            url: "{{ URL::to('admin/approval-nazhir/create-update-approval-pinjam') }}",
            type: "POST",
            dataType: 'json',
            ...
            success: function(response) {
                ...
                if (response.status == 400) {
                    ...
                    window.setTimeout(function() {
                        $('#add-edit-approval-modal').modal('hide');
                        table.draw();
                    }, 500);
                    table.draw();
                }
